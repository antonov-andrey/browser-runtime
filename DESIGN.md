...
После доказанной остановки predecessor platform вызывает run-local `POST /runtime/execution-state-restore` с отдельным Secret-backed одноцелевым credential, недоступным workflow container, и stable identity новой execution. Router сразу закрывает admission новых запросов, прерывает оставшиеся streaming handlers уже остановленного predecessor, дожидается завершения ограниченных candidate operations, останавливает все backend process groups и очищает только mutable profile, candidate, backend-runtime и output roots, не заменяя их mounted directories. HMAC-защищённый marker completed identity хранится вне очищаемых roots, поэтому повтор того же platform request является no-op, а новая execution identity выполняет полное восстановление. Следующий request заново создаёт pair-local profile из уже материализованного platform immutable source. Query parameters и body у restore-команды запрещены.
